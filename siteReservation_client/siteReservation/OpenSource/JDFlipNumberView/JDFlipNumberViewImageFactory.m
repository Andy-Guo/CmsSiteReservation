...
//
#import "JDFlipNumberViewHeader.h"
...
- (void)generateImagesFromBundleNamed:(NSString*)bundleName;
{
    ...
    {
        // create path & image
        NSString *imageName = [NSString stringWithFormat: @"%ld", (long)digit];
        NSString *bundleImageName = [NSString stringWithFormat: @"%@/%@", filename, imageName];
        NSString *path = [[NSBundle mainBundle] pathForResource:bundleImageName ofType:@"png"];
        UIImage *sourceImage = [UIImage imageNamed:bundleImageName];//[[UIImage alloc] initWithContentsOfFile:path];
